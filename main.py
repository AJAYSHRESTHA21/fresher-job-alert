...
def send_no_jobs_email():
    ...
    <html><body style="font-family:Arial,sans-serif;max-width:700px;margin:auto;color:#333;">
      <div style="background:#e53935;padding:24px;border-radius:8px 8px 0 0;text-align:center;">
        <h1 style="color:#fff;margin:0;">📭 No Fresher Jobs Today</h1>
        <p style="color:#ffcdd2;margin:6px 0 0;">{date_str}</p>
      </div>
      <div style="background:#f9f9f9;padding:16px 24px;border:1px solid #ddd;border-top:none;border-radius:0 0 8px 8px;">
        <p>Hi there! 👋</p>
        <p>We scanned <b>30 top service-based companies</b> across <b>Indeed, Naukri & LinkedIn</b> today
        but found <b>no new IT/CS/MCA fresher job openings.</b></p>
        <p>Don't worry — we'll keep checking every day and alert you the moment something comes up! 💪</p>
        <p style="font-size:12px;color:#aaa;margin-top:20px;text-align:center;">
    ...
    msg["Subject"] = f"📭 No Fresher IT Jobs Found Today — {date_str}"
    msg["From"]    = sender
    msg["To"]      = recipient
...
def main():
    ...
    if all_new_jobs:
        ...
        print("📭 No IT/CS/MCA fresher jobs found today. Sending no jobs email...")
        send_no_jobs_email()
